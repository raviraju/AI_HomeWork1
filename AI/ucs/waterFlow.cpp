...
typedef struct node{
	...
	void display()
	{
		cout << "Ravi : state : " << state << " path_cost : " << path_cost << endl;
	}
}node;
...
void ucs_srch(string source, set<string>& destinationNodes, map<string, set<string> >& adjacenyMap, int startTime, ofstream& ofs, map< pipeKey, pipeProperty>& pipes)
{
	...
	parentMap[source]=string("NONE");
	node* srcNodePtr = new node(source, NULL, startTime);
	...
	//set<string> frontier_states;	
	frontier.push(srcNodePtr);frontier_states_path[srcNodePtr->state]=srcNodePtr->path_cost;//frontier_states.insert(srcNodePtr->state);
	cout<< "Ravi : pushing to frontier : " << srcNodePtr->state << " with "<< endl;
	srcNodePtr->display();
	set<string> explored;
	int cur_time = startTime;
	...
	while(1)
	{
		...
		node* poppedNodePtr = frontier.top();frontier.pop();
		//set<string>::iterator frontier_statesItr = frontier_states.find(poppedNodePtr->state);
		//if ( frontier_statesItr != frontier_states.end())
		//	frontier_states.erase(frontier_statesItr);
		cout<< "Ravi : popped from frontier : " << poppedNodePtr->state << endl;
		frontier_states_path.erase(poppedNodePtr->state);
		...
		if(poppedNodePtr->state != source)//first node handling and check for bad_pipe: if bad_pipe:skip and go to next
		{
			...
			if(pipesItr != pipes.end())//found pipe
			{
				...
				cout << "cur_time : " << cur_time << " cur_time_24_hr : " << cur_time_24_hr << endl;
				...
				if((pipesItr->second).no_offPeriods > 0)
				{
					...
					for(set< pair<int,int> >::iterator it = offPeriods.begin(); it!= offPeriods.end(); ++ it)
					{
						...
						{
							cout << "BadPipe : cur_time = " << cur_time << " cur_time_24_hr : " << cur_time_24_hr << " is in OffPeriod : (" << (*it).first << ", " << (*it).second << " ) ";
							badPipe = true;
						}
					}
				}
			...
			if(badPipe)
			{
				...
				continue;//ignore the bad pipe node and move to next
			}
		}
		if (goal_test(poppedNodePtr->state, destinationNodes))
		{
			...
			//write result to output file
			int total_cost = poppedNodePtr->path_cost;
		...
		cur_time += poppedNodePtr->path_cost;
		cout<< "Ravi : update cur_time to  : " << cur_time << endl;
		explored.insert(poppedNodePtr->state);
		cout<< "Ravi : Added to explored : " << poppedNodePtr->state << endl;
		if(poppedNodePtr->state != source)
		{			
			parentMap[poppedNodePtr->state]=(poppedNodePtr->parent)->state;
		...
		if(adjacenyMapItr != adjacenyMap.end())
		{
			...
			for(set<string>::iterator it = children.begin(); it!= children.end(); ++ it)
			{
				...
				{	
					...
					//cout<< "Ravi : update : parentMap[" << child << "] = " << poppedNodePtr->state << endl; 
					//parentMap[child]=poppedNodePtr->state;
				...
				else if (found_in_frontier_states_path(child, frontier_states_path))
				{
					...
					if(pipesItr != pipes.end())//found pipe
					{
						...
						cout<< "Ravi : found new_pipe_length : " << new_pipe_length << endl;
					}
					
					if(existing_path_cost > (poppedNodePtr->path_cost + new_pipe_length))
					{
						...
						{  
							...
							if(n->state == child)
							{
								cout << "Ravi : found and deleting..." << endl;
								delete n;
								/////parentMap[child]= poppedNodePtr->state;
								break;
							}
							...
							{
								cout << "pushing to temp_frontier : " << n->state << endl;
						...
						node* childPtr = new node(child, poppedNodePtr, poppedNodePtr->path_cost + new_pipe_length);
						frontier.push(childPtr);
						cout << "Updating node " << childPtr->state <<" with existing path_cost : " << existing_path_cost << " to optimal path_cost : " << (childPtr->path_cost) << endl;
						cout<<"frontier size after add : " << frontier.size()<<endl;
					}
				}
			}
		}
